/**
 * Migration: Phase 1 â€” Add missing columns for Task Detail features
            ...
            db.query(m.sql, (err) => {
                if (err) {
                    if (err.code === "ER_DUP_FIELDNAME" || err.code === "ER_TABLE_EXISTS_ERROR" ||
                        ...
                        console.log(`  â­ï¸  SKIP (already exists): ${m.name}`);
                    } else {
                        console.error(`  âŒ ERROR: ${m.name}\n     ${err.message}`);
                    }
                } else {
                    console.log(`  âœ… OK: ${m.name}`);
                }
                resolve();
            });
        });
...
db.connect((err) => {
    if (err) {
        console.error("âŒ Cannot connect to DB:", err.message);
        process.exit(1);
    }
